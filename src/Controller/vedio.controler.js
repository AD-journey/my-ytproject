...
pipeline.push({ $match: { isPublished: true } });
  ...
  const video = await Vedio.aggregate(pipeline);
  ...
  //  const vedio = await Vedio.aggregatePaginate(video, options);
...
const publishAVideo = asyncHandler(async (req, res) => {
  ...
  const videoFile = await fileCoudinary(videoFileLocalPath);
  const thumbnail = await fileCoudinary(thumbnailLocalPath);
  ...
  const videoUploaded = await Vedio.findById(video._id);
...
const getVedioById = asyncHandler(async (req, res) => {
  ...
  await Vedio.findByIdAndUpdate(videoId, {
    $inc: {
      views: 1,
    },
  });
...
const deleteVideo = asyncHandler(async (req, res) => {
  ...
  const videoDeleted = await Vedio.findByIdAndDelete(video?._id);
...
const togglePublishStatus = asyncHandler(async (req, res) => {
  ...
  const toggledVideoPublish = await Vedio.findByIdAndUpdate(
    videoId,
    {
      $set: {
